...
/// Enum representing the runtime value of a JavaScript expression.
...
/// Struct representing a JavaScript function.
...
/// Struct representing a JavaScript variables.
#[derive(Debug, Clone)]
    ...
    fn update_variable(&mut self, name: String, value: Option<RuntimeValue>) {
        for i in 0..self.variables.len() {
            if self.variables[i].0 == name {
                ...
                return self.variables.push((name, value));
            }
        }
    }
}

/// Struct representing a JavaScript runtime.
#[derive(Debug, Clone)]
        ...
        match node.borrow() {
            ...
            Node::CallExpression { callee, arguments } => {
                ...
                let function = {
                    ...
                    match f {
                        Some(f) => f,
                        None => panic!("Function {:?} does not exist.", callee),
                    }
                };
...
mod tests {
    ...
    fn test_run(input: String, expected: Vec<Option<RuntimeValue>>) {
    ...
    #[test]
    fn test_num() {
        test_run("42".to_string(), [Some(RuntimeValue::Number(42))].to_vec())
    }
    #[test]
    fn test_add_nums() {
        test_run("1+2".to_string(), [Some(RuntimeValue::Number(3))].to_vec())
    }
    #[test]
    fn test_sub_nums() {
        test_run("2-1".to_string(), [Some(RuntimeValue::Number(1))].to_vec())
    }
    #[test]
    fn test_assign_variable() {
        test_run("var foo=42;".to_string(), [None].to_vec())
    }
    #[test]
    fn test_add_variable_and_num() {
        test_run(
            "var foo=42;foo+1".to_string(),
            [None, Some(RuntimeValue::Number(43))].to_vec(),
        )
    }
    #[test]
    fn test_reassign_variable() {
        test_run(
            "var foo=42;foo=1;foo".to_string(),
            [None, None, Some(RuntimeValue::Number(1))].to_vec(),
        )
    }
    #[test]
    fn test_add_function_and_num() {
        test_run(
            "function foo(){return 42;}foo()+1".to_string(),
            [None, Some(RuntimeValue::Number(43))].to_vec(),
        )
    }
    #[test]
    fn test_define_function_with_args() {
        test_run(
            "function foo(a,b){return a+b;}foo(1,2)+3".to_string(),
            [None, Some(RuntimeValue::Number(6))].to_vec(),
        )
    }
    #[test]
    fn test_local_variable() {
        test_run(
            "var a=42;function foo(){var a=1;return a;}foo()+a".to_string(),
            [None, None, Some(RuntimeValue::Number(43))].to_vec(),
        )
    }
}
